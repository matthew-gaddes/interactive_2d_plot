        ...
        if event.inaxes == axes1:                                                       # determine if the mouse is in the axes
            ...
            if cont:                                                                    # if on point
                ...
                if arrow_lengths[0] > 0 and arrow_lengths[1] > 0:                                                                    # top right quadrant
                    ...
                    inset_axes = fig.add_axes([fig_x - inset_axes_side, fig_y, inset_axes_side, inset_axes_side])                    # create the inset axes, nudged in x direction
                elif arrow_lengths[0] > 0 and arrow_lengths[1] < 0:                                                                  # lower right quadrant
                    inset_axes = fig.add_axes([fig_x, fig_y - inset_axes_side, inset_axes_side, inset_axes_side])                    # create the inset axes, nudged in y direction
                else:
                    inset_axes = fig.add_axes([fig_x - inset_axes_side, fig_y - inset_axes_side, inset_axes_side, inset_axes_side])  # create the inset axes, nudged in both x and y
...
spatial_data = {'images' : spatial_maps_r2,
                'mask'   : mask}

exploratory_2d_plot(xy, c, spatial_data = spatial_data, inset_axes_side = 0.1, arrow_length = 0.05, figsize = (10,6))
